...
def get_sql_from_llm(user_question):
    """
    Uses OpenAI to convert a user's question into a SQL query.
    """
    system_prompt = f"""
    You are an expert SQL assistant. Your task is to generate a SQL query based on a user's question.
...
def get_response_from_llm(user_question, db_results):
    """
    Uses OpenAI to generate a natural language response based on the user's question and database results.
    """
    system_prompt = """
    ...
    Your task is to provide a clear, friendly, and concise answer to the user's question based on the provided database results.
...
def chat():
    ...
    sql_query = get_sql_from_llm(user_question)
    if not sql_query:
    ...
    final_response = get_response_from_llm(user_question, db_results)
